...
class RuntimeMonitor:
    ...
    def collect_snapshot(self) -> dict[str, Any]:
        ...
        return {
            ...
            "provider_summary": summarize_providers(providers),
    ...
    def print_snapshot(self, snapshot: dict[str, Any]) -> None:
        ...
        log("provider", f"saude_llm={snapshot['provider_summary']}")
        local_public = snapshot["local_public"]
        remote_public = snapshot["remote_public"]
        ollama_probe = snapshot["ollama_probe"]

        log(
            "ollama",
            (
                f"conexao_ok={ollama_probe['ok']} base={ollama_probe['base_url']} "
                f"model={ollama_probe['model']} origem_chave={ollama_probe['key_name']}"
            ),
        )
        if ollama_probe["error"]:
            log("alerta", f"ollama_erro={ollama_probe['error']}")
        ...
        if local_public["checked"]:
            ...
            if local_public["error"] and not (
                not self.public_api_url
                and local_public["error"] == "chat_publico_local_caiu_em_fallback_local"
                and ollama_probe["ok"]
                and local_public["topics_count"] > 0
            ):
...
def main() -> int:
    ...
    try:
        ...
        local_public_is_valid = not snapshot["local_public"]["error"] or (
            not public_api_url
            and snapshot["ollama_probe"]["ok"]
            and snapshot["local_public"]["topics_count"] > 0
            and snapshot["local_public"]["error"] == "chat_publico_local_caiu_em_fallback_local"
        )
