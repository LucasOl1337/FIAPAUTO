...
import {
  ...
  listPublishedTopics,
  readPublishedAsset,
} from '@fiapauto/server-core/public/service'
import {
  readPublicSession,
  signInPublicGuest,
  signInPublicUser,
  ...
  signUpPublicUser,
  touchPublicSession,
} from '@fiapauto/server-core/public/auth'
import { appendPublicChatTraceEvent, buildPublicTraceEvent } from '@fiapauto/server-core/public/monitorStore'
import { corsHeaders, readClientIp, readJsonBody, sendJson } from '../../../backend/apis/http.ts'
...
export async function handlePublicApiRoutes(
  ...
  if (request.method === 'GET' && requestUrl.pathname === '/api/public/topics') {
    sendJson(response, 200, { topics: await listPublishedTopics() })
    return true
  }
