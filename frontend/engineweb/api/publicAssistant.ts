...
function classifyQuestionIntent(question: string) {
  ...
  if (/\b(prazo|deadline|vence|data)\b/i.test(normalized)) return 'deadline'
  if (/\b(entrega|entregavel|entregaveis|deliverable|preciso entregar)\b/i.test(normalized)) return 'deliverable'
  if (/\b(resumo|explique|sobre o que e|do que se trata)\b/i.test(normalized)) return 'summary'
  if (/\b(proximos passos|checklist|o que fazer agora)\b/i.test(normalized)) return 'next_steps'
  if (/\b(nota|avaliacao|criterio|formato|abnt)\b/i.test(normalized)) return 'grading'
  if (/\b(item|parte|topico|numero)\b/i.test(normalized)) return 'numbered_item'
  return 'summary'
}
