...
import { postLlmChat, type LlmDocument, type LlmImage } from '../connections/llm/llmClient.ts'
...
export async function answerPublishedTopicQuestion(input: {
  ...
  try {
    ...
    if (primaryPayload.qualityStatus !== 'accepted') {
      ...
      try {
        const retry = await requestPublicQwenChat({
...
async function requestPublicQwenChat(input: {
  ...
  return {
    ...
    providerUsed: 'qwen' as const,
    strategyUsed: 'rag_llm' as const,
    fallbackLevel: 0,
  }
}
