...
export async function handlePublicRoutes(
  ...
  if (request.method === 'POST' && requestUrl.pathname === '/api/public/chat/topic') {
    ...
    console.log(`[public-chat] start topic=${topicId} model=${PUBLIC_QWEN_MODEL} question="${compactLogText(question)}"`)
    try {
      const result = await askPublishedTopic({ topicId, question })
      console.log(
        `[public-chat] done topic=${topicId} provider=${result.providerUsed} strategy=${result.strategyUsed} quality=${result.qualityStatus} pass=${result.answeredByPass}`,
      )
      sendJson(response, 200, result)
    } catch (error) {
      const message = error instanceof Error ? error.message : 'public_chat_failed'
      console.error(`[public-chat] error topic=${topicId} model=${PUBLIC_QWEN_MODEL} error=${message}`)
